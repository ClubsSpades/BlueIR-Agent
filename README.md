...
当前版本定位为比赛 MVP：通过 DeepSeek API、结构化安全 Skill、只读分析工具和轻量 Web UI，实现告警分诊、IOC 提取、时间线整理、Webshell 日志分析、Windows 登录爆破分析、Linux 应急证据分析、PCAP/EVTX 安全预分析、MITRE ATT&CK 映射和 Markdown 应急报告生成。
...
推荐直接使用 [attachments](attachments/) 目录里的仿真附件测试 v0.3 Web UI：
...
当前真实接入仍然只有 DeepSeek，但系统已经有角色配置层。默认角色包括：
